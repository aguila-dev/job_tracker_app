...
type USAData = Record<string, StateData>
...
export const isLocationInUS = (location: string): boolean => {
  const [city, statePart] = location.split(',').map((part) => part.trim())
  console.log({ city, statePart })

  if (!city || !statePart) {
    return false
  }

  // Normalize state part (could be full name or abbreviation)
  const stateAbbr = Object.keys(usaData).find(
    (key) =>
      usaData[key].state.toLowerCase() === statePart?.toLowerCase() ||
      key.toLowerCase() === statePart?.toLowerCase()
  )
  ...
  // Check if city exists in the state's city list
  const stateData = usaData[stateAbbr]
